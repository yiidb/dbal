...
use Yii\DBAL\Query\Expressions\Conditions\ArrowCondition;
use Yii\DBAL\Query\Expressions\Conditions\ColumnSimpleCondition;
use Yii\DBAL\Query\Expressions\Conditions\CompositeCondition;
use Yii\DBAL\Query\Expressions\Conditions\InCondition;
use Yii\DBAL\Query\Expressions\Conditions\NotCondition;
use Yii\DBAL\Query\Expressions\Conditions\ParamsCondition;
use Yii\DBAL\Query\Expressions\Conditions\RawStringCondition;
use Yii\DBAL\Query\Expressions\Conditions\SimpleCondition;
use Yii\DBAL\Query\Expressions\Conditions\StringCondition;
...
final class SimpleConditionBuilder implements ConditionBuilderInterface
{
    ...
    public function build(
        ...
        return $cond instanceof ConditionInterface ? $cond->build($exprBuilder) : $cond;
    }
    ...
    /**
     * @psalm-param non-empty-array $cond
     * @return array<string|ExpressionInterface|ConditionInterface>
     */
    private function prepareCondArr(
        ...
        if ($firstKey === 0 && is_string($condName = $cond[0])) {
            ...
            if ($condClass === null) {
                if (!array_is_list($cond) || !in_array($cond[1], SimpleCondition::SUPPORT_OPERATORS, true)) {
                    throw new InvalidExpressionFormatException(sprintf('Unknown condition name "%s".', $condName));
                }
                /** @var array{1: string} $cond */
